    ...
    for(int i = 1; i < argc; i++){
      if(!skipEdC(argv, i)){
        if(!strcmp(argv[i], "icol") 
            && col == atoi(argv[i+1]) 
            && firstColN >= atoi(argv[i+1]))
          colsToPt++;
        else if(!strcmp(argv[i], "acol"))
          acolPresent = true;
      }
    }
    if(!acolPresent && deldCols >= firstColN) 
      colsToPt--;
...
void irowArow(int argc, char *argv[], char *del, int row, int firstColN, bool arow){
  ...
  for(int i = 0; i < argc; i++){
    if(!skipEdC(argv, i)){
      ...
      //TODO DO FUNKCIE NASLEDUJUCE TRI RIADKY?
  ...
  for(int i = 1; i < argc; i++){
    if(!skipEdC(argv, i)){
      if(/*i != delPlc && */!strcmp(argv[i], "irow") && arow == false/*&& row == atoi(argv[i+1])*/){
        //If irow is called
...
bool doIEdFn(char slC[MAXCA][MAXSL], char buffer[], char *del, int row){
  char *tolptr;
  ...
  if(!slC[0][0])
    ...
    if(!strcmp(slC[0], "beginswith")){
      ...
      for(int/*size_t*/ i = 0; i < arg2Len/*strlen(slC[2])*/; i++)
...
bool pcNumsAndChars(char pcC[MAXCA][MAXSL], char token[2*MAXSL]){
  if(!strcmp(pcC[0], "tolower")){
    ...
    for(/*size_t*/int i = 0; i < tokLen/*strlen(token)*/; i++)
...
bool cpSpMvFn(char buffer[MAXRL], char token[2*MAXSL], 
  ...
  for(int i = 0; i < MAXRL; i++){
    ...
    if(actCol == pcCA2){
      ...
      while((i + j + 1) < bufferLen/*(int)strlen(buffer)*/ 
          && buffer[i+j] != del[0] 
          && buffer[i+j] != '\0'){
  ...
  for(int i = 0; i < csNum; i++){
    if(!strcmp(pcC[0], cs[i]) && strtol(pcC[1], &tolptr, 10)){
      ...
      if(arg1 == col){
        ...
        int bufferLen = (int)strlen(buffer);
        for(int j = 0; j < bufferLen/*(int)strlen(buffer)*/; j++){
...
int pcTheTab(char buffer[], char *del, char token[2*MAXSL], char tempTok[2*MAXSL], 
  ...
  if(doIEdFnRet != 1 && pcC[0][0] == 'r' && strcmp(pcC[0], "rseq")
      && (row == (atoi(pcC[3]) + 1) || -row == (atoi(pcC[3]) + 1)))
    doIEdFnRet = 1; 
    //For row processing commands (except for rseq)
    //If the row is M+1 (M as the arg), we edit anyways. 
  else if(!strcmp(pcC[0], "rseq") && (row > 1 || row < 1) 
      && (row == (atoi(pcC[3]) + 1) || -row == (atoi(pcC[3]) + 1)))
    doIEdFnRet = 1;
  //TODO komentar
...
int main(int argc, char *argv[]){
  ...
  //Get info about where the deliter chars are in argv
  char *del = " ";
  if(delPlc < 0) 
    return -delPlc; 
