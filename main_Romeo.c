...
int isCmd(int argc, char *argv[], int delPlc, int *j, int cmdAsN[], char cmdArr[][mxCL], int *cmdN, char CA[][mxSL]){
...
bool doIEdFn(char slC[mxCAsN][mxSL], char buffer[], char *del, int rowN){
  char *strtolptr;
            ...
            if(actCol >= arg2 && actCol <= arg3){
              ...
              if(!strcmp(pcC[0], "csum"))
                ...
                if(!init && !strtodptr[0]){ //if this is the first cycle (!init) and there is a number in the column, copy it.
                  outVal = tokVal;
                  init = true;
                }else if(!strcmp(pcC[0], "cmin") && outVal > tokVal && !strtodptr[0])
                  outVal = tokVal;
                else if(!strcmp(pcC[0], "cmax") && outVal < tokVal && !strtodptr[0])
                  outVal = tokVal;
...
int pcTheTab(char buffer[], char *del, char token[2*mxSL], char tempToken[2*mxSL], int rowN, int colN, 
  ...
  int doIEdFnRet = doIEdFn(slC, buffer, del, rowN);
...
void irowArow(int argc, char *argv[], int delPlc, char *del, int rowN, int firstColN, bool arow){
  ...
  for(int i = 1; i < argc; i++){
    if(!skipEdC(argv, i)){
      if(i != delPlc && !strcmp(argv[i], "irow") && arow == false/*&& rowN == atoi(argv[i+1])*/){
...
bool dcolDcols(int argc, char *argv[], int colN){
  for(int i = 0; i < argc; i++){
    if(!skipEdC(argv, i)){
  ...
  while(1){
    ...
    for(size_t k = 0; k < strlen(buffer); k++){
      ...
      if(buffer[k] != del[0] && buffer[k] != '\n'){ //if a char isn't a delim and neither a line break
        ...
        if(edCN && dcolDcols(argc, argv, colN)){
          dontPtNextChar = true;
          if(colN == 1)
            dontPtNextDel = true;
        }
