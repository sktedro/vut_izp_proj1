...
void replaceDelims(char buffer[mxRwL], char *del, int rowN, int *firstColN){
...
bool isCmd(int argc, char *argv[], int delPlc, int j, int cmdAsN[], char cmdArr[][mxCL], int *cmdN){
  char exceptions[][mxCL] = {"cset", "beginswith", "contains"};//the only ones that can have a string as an argument
  int numOfExceptions = 3;
  if(argv[j]){
    if((j > 2 && (!strcmp(argv[j - 2], exceptions[0]) || !strcmp(argv[j - 2], exceptions[1]) || !strcmp(argv[j - 2], exceptions[2])))
        || (j > 1 && !strcmp(argv[j - 1], "-d")))
      return true; //dont scan the command, if there's an exception two places before
    ...
    for(int i = 0; cmdArr[i][0]; i++){
      if(!strcmp(argv[j], cmdArr[i])){
        if((j + cmdAsN[i]) >= argc || (delPlc > j && delPlc <= (j + cmdAsN[i] + 1))){
          //if(strcmp(argv[j - 2], exceptions[0]) && strcmp(argv[j - 2], exceptions[1]) && strcmp(argv[j - 2], exceptions[2])){
        ...
        for(int l = 0; l < cmdAsN[i]; l++){
          ...
          bool BArg = (!strcmp(argv[j], "rseq") && l == 3) || (!strcmp(argv[j], "cseq") && l == 2);
...
/*int*/ bool doIEdFn(char slC[mxCAsN][mxSL], char buffer[], char *del, int rowN/*, int colN*/){
  char *strtolptr;
...
int rseq(char pcC[mxCAsN][mxSL], int rowN, int colN, double *rSAMMCVal, bool *rseqInit){
  ...
  if(colN == arg[0] && rowN >= arg[1] && rowN <= arg[2]){
    //pozit rSAMMCAvgCt ako init. rows 2 3 rseq 1 - 10 nefunguje: rata od
    //jednotky
    if(*rseqInit == false){
      *rseqInit = true;
      *rSAMMCVal = arg4; 
...
int pcTheTab(char buffer[], char *del, char token[2*mxSL], char tempToken[2*mxSL], int rowN, int colN, 
  ...
  //if(doIEdFnRet == 2)
    //return 0;
  //if(doIEdFnRet != 1 && pcC[0][0] == 'r' && (rowN == (atoi(pcC[3]) + 1) || -rowN == (atoi(pcC[3]) + 1) || pcC[3][0] == '-'))
    //doIEdFnRet = 1; 
  if(doIEdFnRet != 1 && pcC[0][0] == 'r' && strcmp(pcC[0], "rseq")
      && (rowN == (atoi(pcC[3]) + 1) || -rowN == (atoi(pcC[3]) + 1))/* && (rowN > 1 || rowN < 1)*/)
...
void icolAcol(char *token, long int colN, int firstColN, int argc, char *argv[], char *del/*, int delPlc*/){
  char *strtolptr;
...
int pcAndSlCFn(int argc, char *argv[], int delPlc, int *edCN, int *pcCN, 
  ...
  for(int i = 1; i < argc; i++){
    ...
    if(!isCmd(argc, argv, delPlc, i, edCAN, edCArr, edCN)
      || !isCmd(argc, argv, delPlc, i, pcCAN, pcCArr, pcCN)
      || !isCmd(argc, argv, delPlc, i, slCAN, slCArr, &slCN))
      return -3;
  }
  if(*pcCN == 1)
...
int main(int argc, char *argv[]){
  ...
  while(1){
    ...
    if(rowN >= 0)
      ...
      rowN--;
    replaceDelims(buffer, del, rowN, &firstColN);
    irow(argc, argv, delPlc, del, rowN, firstColN);
    ...
    for(size_t k = 0; k < strlen(buffer); k++){
      ...
      if(buffer[k] != del[0] && buffer[k] != '\n'){ //if a char isn't a delim and neither a line break
        ...
        if(pcCN == 1){ //if a table processing command was inputed by the user
          ...
          icolAcol(token, colN, firstColN, argc, argv, del/*, delPlc*/);
