/*
** IZP - Project 1
** Patrik Skalos, xskalo01, group #15, assistant Skandera
**
...
**
** If a command which is supposed to work with numbers (int, round, csum,
** rsum,...) approaches a non-numeric cell, it simply skips it and does not
** stop the program. For commands that calculate a value based on the columns
** (csum, ravg,...), if there was not a single number processed, they write NaN
** (not a number) into the cell where the calculated value would be.
**
** Return values from main:
  ...
  ** 11 the input table is empty
  ** 12 the user is trying to edit a single column twice
**
**
** Variables:
  ** S = string
...
int errHandler(int errCode, char *str){
  switch(errCode){
    case 1:
    ...
    case 7:
      ...
      break;
    case 8:
      fprintf(stderr, "ONE OF YOUR ROWS IS TOO LONG! THE PROGRAM WILL NOW EXIT.\n");
      break;
    case 9:
    ...
    case 10:
      ...
      break;
    case 11:
      fprintf(stderr, "THE TABLE YOU PROVIDED IS EMPTY! THE PROGRAM WILL NOW EXIT.\n");
      break;
    case 12:
...
int getDel(int argc, char *argv[]){
  if(argc > MAXC)
    return errHandler(2, NULL); //Too many arguments
  int delPlace = 0;
  for(int i = 1; i < argc; i++){
    if(strlen(argv[i]) >= MAXAL)
      return errHandler(1, NULL); //Entered argument is too long
    if(strstr(argv[i], "-d") && (i + 1) < argc && !delPlace)
      delPlace = i + 1; //i is the place of -d, i+1 is the place of delimiters 
  }
  return delPlace;
}
...
bool replaceDels(char buffer[MAXRL], char *del, int row, int *firstColN){
  ...
  if(row > 1 && dels != *firstColN){
    errHandler(9, NULL); //Not all rows have the same amount of columns
    return 0;
  }
  return true;
}
...
int getArgs(int argc, char *argv[], int delPlc, int *edCN, 
  ...
  for(int i = 1; i < argc; i++){
    ...
    int isCmdEdRet = isCmdEdRet = isCmd(argc, argv, delPlc, &i, edCAN, edCArr, edCN, edC);
    if(!isCmdEdRet|| !isCmdPcRet || !isCmdSlRet )
      return -3;
  }
  if((*pcCN || *slCN) && *edCN)
    return errHandler(7, NULL); //Editing and processing the tab in one execution
  if(*edCN && dupDcol(argc, argv))
    return errHandler(12, NULL); //User is trying to delete the same column several times
  if(*pcCN > 1 || *slCN > 1)
    return errHandler(6, NULL); //More than one processing or selection command
  if(*pcCN && !strcmp(slC[0], "rows") && pcC[0][0] == 'r')
    return errHandler(10, pcC[0]); //Using rows along with rseq is forbidden
  if(*pcCN != 0 && *edCN != 0) 
    return 0; //If there is nothing to be done with the table
  if(*edCN != 0)
    return 1;
  if(*pcCN == 1)
    return 2;
...
bool pcNumsAndChars(char pcC[MAXCA][MAXSL], char token[2*MAXSL]){
  if(!strcmp(pcC[0], "tolower")){
    ...
    char *todptr = NULL;
    double num = strtod(token, &todptr);
    num += (num > 0) ? (+ 0.5) : (- 0.5);
    int intN = num;
    if(!todptr[0])
    ...
    bool containsOneDot = false; //To check how many '.' there are in the string
    for(int i = 0; i < tokLen; i++){
      if(token[i] == '.'){
        if(containsOneDot)
          return false;
          //Contains two dots.. therefore it is not a valid number
        containsOneDot = true;
      }
      ...
      if(token[i] == '-' && i != 0)
        return false;
        //There is a '-' and it is not at the beginning of the string
    }
...
int colPcCsFn(double *colPcCsVal, char buffer[MAXRL], 
  ...
  for(int i = 0; i < csNum; i++){
    if(!strcmp(pcC[0], cs[i]) && strtol(pcC[1], &tolptr, 10)){
      bool retNULL = true, retNULLHelper = false; 
      ...
      if(arg1 == col){
        ...
        if((arg1 >= arg2 && arg1 <= arg3) || tolptr2[0] || tolptr3[0])
          return errHandler(4, pcC[0]); //Wrong column arguments for column processing commands
        bool init = false;
        int actCol = 1;
        int avgCt = 0;
        int k = 0;
...
int rowPcCsFn(char token[2*MAXSL], char pcC[MAXCA][MAXSL], int col, int row, 
  ...
  if(col == arg1){
    ...
    if(row == arg3 + 1){ 
      ...
      if(*rowPcCsCt == 0) 
        if(!strcmp(pcC[0], "rmin") || !strcmp(pcC[0], "rmax") 
            || !strcmp(pcC[0], "ravg"))
          return 6; 
...
int pcTheTab(char buffer[], char *del, char token[2*MAXSL], char tempTok[2*MAXSL], 
  ...
  if(doIEdFnRet == 1){
    if(strcmp(pcC[0], "rseq") && pcC[0][0] == 'r'){
      char *tolptr = NULL;
      int tempPcCA3 = strtol(pcC[3], &tolptr, 10) + 1;
      if(row == tempPcCA3 || -row == tempPcCA3)
        doIEdFnRet = 2;
...
/*
** 'main' function (much longer than intended. Sorry)
*/
...
int main(int argc, char *argv[]){
  ...
  int slCN = 0; //...
  char pcC[MAXCA][MAXSL] = {{0}};
  char slC[MAXCA][MAXSL] = {{0}};
  ...
  if(tempRet == 0){
    ...
    if(row == 0)
      return -errHandler(11, NULL); //The table is empty
    return 0;
  }
  int finish = 0; 
  ...
  int rowPcCsCt = 0;
  bool rseqInit = false;
  if(!fgets(prevBuffer, MAXRL, stdin))
    return -errHandler(11, NULL);
  while(1){
    strcpy(buffer, prevBuffer); 
    //Load the row into the buffer
    if(!strchr(buffer, '\n'))
      return -errHandler(8, NULL);
    ...
    for(size_t k = 0; k < strlen(buffer); k++){
      ...
      if(buffer[k] != del[0] && buffer[k] != '\n'){ 
        ...
        if(pcCN == 1){ 
          ...
          int pcTheTabRet = pcTheTab(buffer, del, token, tempTok, row, col, 
              pcC, slC, &colPcCsVal, &rowPcCsVal, &rowPcCsCt, &rseqInit);
