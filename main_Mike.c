...
int doIEditFn(char selCmd[maxCmdLen], char selCmdArgs[maxCmdArgsNum][maxStrLen], char token[maxStrLen], int rowNum, int colNum){
  bool doIEdit = false;
  char *strtolptr;
  int arg1 = strtol(selCmdArgs[0], &strtolptr, 10);
  char *strtolptr2;
  int arg2 = strtol(selCmdArgs[1], &strtolptr2, 10);
  if(!selCmd[0]){
    doIEdit = true;
    ...
    if(selCmdArgs[1][0] == '-'){
      if(selCmdArgs[0][0] == '-'){ 
        if(rowNum < 0)
          doIEdit = true;
      }else if(rowNum >= arg1 || (rowNum < 0 && -rowNum >= arg1))
        doIEdit = true;
    }
    else if(1){
      if(rowNum < 0)
  ...
  }else if(!strcmp(selCmd, "beginswith") && colNum == arg1 && token == strstr(token, selCmdArgs[1])){
    doIEdit = true;     
  }else if(!strcmp(selCmd, "contains") && colNum == arg1 && strstr(token, selCmdArgs[1])){
    doIEdit = true;     
  }
  if(selCmd[0] && strtolptr[0] && strtolptr[0] != '-')
    return 2;
  if(doIEdit == true)
    return 1;
  else 
...
int procTheTab(char buffer[], char mainDel[], char token[maxStrLen], char tempToken[maxStrLen], 
  ...
  char *strtolptr;
  bool doIEdit = false;
  int doIEditFnRet = doIEditFn(selCmd,selCmdArgs, token, rowNum, colNum);
  if(doIEditFnRet == 2)
    return 0;
  else if(doIEditFnRet == 1)
    doIEdit = true;
  if(doIEdit){
  ...
  for(int i = 1; i < argc; i++){
    if(i != delPlace){
      if(!strcmp(argv[i], "drow") || !strcmp(argv[i], "drows")){
        ...
        if(strtolptr[0])
          return 2;
        if(!strcmp(argv[i], "drow") && rowNum == arg1)
          return 1;
...
int procAndSelCmdsFn(int argc, char *argv[], int delPlace, int editCmdsNum, int procCmdsNum, 
    char procCmd[maxCmdLen], char procCmdArgs[maxCmdArgsNum][maxStrLen], char selCmd[maxCmdLen], char selCmdArgs[3][maxStrLen]){
...
int main(int argc, char *argv[]){
  ...
  mainDel[0] = delsArr[0];
  if(1){
    int tempRet = procAndSelCmdsFn(argc, argv, delPlace, editCmdsNum, procCmdsNum, procCmd, procCmdArgs, selCmd, selCmdArgs);
    if(tempRet != 1)
      return tempRet;
  }
  ...
  while(1){
    ...
    if(!fgets(prevBuffer, maxRowLen, stdin) && !finish){ //load a line into the buffer. If the line is empty, it means the previous row was the last one
      rowNum = -rowNum; //to indicate that this is the last row, we set rowNum to negative (last run of the while cycle)
      finish++;
      continue;
    }
    else if(finish == 2)
    ...
    int drowsRet = drows(argc, argv, delPlace, rowNum);
      if(drowsRet == 2)
        return 0;
      else if(drowsRet == 1){
        colNum = prevColNum;
